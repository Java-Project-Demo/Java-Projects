...
const XuatKhoPage = () => {
  ...
  const availableProducts = useMemo(() => {
    ...
    return products
      .map((p) => {
        let realStock = p.currentStock
        if (p.hasImei) {
          realStock = p.hasImei ? (p.items.filter((item) => item.status == 'AVAILABLE').length ?? 0) : p.currentStock
        }
        return { ...p, currentStock: realStock }
      })
      .filter((p) => p.currentStock > 0 && p.status === 'ACTIVE' && !cartIds.includes(p.id))
  }, [products, cart])
  ...
  const addToCart = () => {
    ...
    if (cart.some((c) => c.productId === pid)) {
      void message.warning(t('export.cartAlreadyIn'))
      return
    }
    setCart((prev) => [
      ...prev,
      { productId: p.id, name: p.name, priceExport: p.priceExport, quantity: 1, stock: p.currentStock }
    ])
  ...
  const handleSubmit = () => {
    form.validateFields(['customerName', 'customerPhone', 'paymentMethod']).then(async (values) => {
      if (cart.length === 0) {
        void message.error(t('export.validation.emptyCart'))
        return
      }
  ...
  const cartColumns: ColumnsType<CartRow> = [
      ...
      render: (v: string, r) => (
        <div>
          ...
          <Text type='secondary' style={{ fontSize: 11 }}>
            {t('export.cartStockInfo', { count: r.stock })}
          </Text>
        </div>
      )
    },
    {
      title: t('export.cartColPrice'),
    ...
    {
      title: t('export.cartColSum'),
      key: 'sum',
      width: 140,
      render: (_, r) => <CurrencyDisplay value={r.priceExport * r.quantity} color={PRIMARY} size='small' />
    },
    {
      title: '',
      key: 'del',
      ...
      render: (_, r) => (
        <Button
          ...
          size='small'
          icon={<DeleteOutlined />}
          onClick={() => setCart((prev) => prev.filter((c) => c.productId !== r.productId))}
        />
      )
    }
  ]
  ...
  return (
    <div>
          ...
          >
            <Space>
              <Text strong style={{ color: '#52c41a' }}>
                {t('export.lastOrderTag', { id: lastOrder.id, customer: lastOrder.customerName })}
              </Text>
            ...
            <Space>
              ...
              {lastOrder.status === 'PENDING' && (
                <Button size='small' danger icon={<StopOutlined />} loading={cancelling} onClick={handleCancelOrder}>
                  {t('export.cancelOrder')}
                </Button>
              )}
              <Button type='text' size='small' onClick={() => setLastOrder(null)}>
      ...
      <Row gutter={[20, 20]}>
            ...
            extra={
              <Space>
                <Select
                  ...
                  placeholder={t('export.cartProductSelect')}
                  style={{ width: 280 }}
                  ...
                  options={availableProducts.map((p) => ({
                    value: p.id,
                    label: t('export.cartProductOption', { name: p.name, stock: p.currentStock })
                  }))}
                  allowClear
                />
                <Button type='primary' icon={<PlusOutlined />} onClick={addToCart}>
                  {t('export.cartAddButton')}
                </Button>
              </Space>
            }
          >
        ...
        <Col xs={24} lg={10}>
          <Card
            title={t('export.customerCardTitle')}
            style={{ borderRadius: 12, boxShadow: '0 2px 10px rgba(0,0,0,0.07)' }}
          >
            <Form form={form} layout='vertical'>
              <Form.Item
                label={t('export.customerPhone')}
                name='customerPhone'
                tooltip={t('export.lookupHint')}
                rules={[{ required: true, message: t('export.customerPhoneRequired') }]}
              >
                <Input
                  placeholder={t('export.customerPhonePlaceholder')}
                  suffix={
                    lookingUp ? (
                      <Text type='secondary' style={{ fontSize: 11 }}>
                        {t('export.lookupSearching')}
                      </Text>
                    ) : null
                  }
                />
              ...
              <Form.Item
                label={t('export.customerName')}
                name='customerName'
                rules={[{ required: true, message: t('export.customerNameRequired') }]}
              >
                <Input placeholder={t('export.customerNamePlaceholder')} />
              </Form.Item>
              <Form.Item label={t('export.customerAddress')} name='customerAddress'>
                <Input.TextArea rows={2} />
              </Form.Item>
              <Form.Item
                label={t('export.paymentMethod')}
                name='paymentMethod'
                rules={[{ required: true, message: t('export.paymentMethodRequired') }]}
              >
                <Select
                  options={[
      ...
      <Modal
        title={
          <Space>
            <PrinterOutlined />
            <span>{t('export.printTitle', { id: lastOrder?.id ?? '' })}</span>
          </Space>
        }
        open={printOpen}
        ...
        {lastOrder && (
          <div ref={printRef} style={{ fontFamily: 'monospace', fontSize: 13 }}>
            ...
            <div style={{ marginBottom: 8 }}>
              <Text strong>{t('export.slip.orderNumber')}</Text> {lastOrder.id}
            </div>
            <div style={{ marginBottom: 8 }}>
              <Text strong>{t('export.slip.customer')}</Text> {lastOrder.customerName}
            </div>
            <div style={{ marginBottom: 8 }}>
              <Text strong>{t('export.slip.phone')}</Text> {lastOrder.customerPhone}
            </div>
            <div style={{ marginBottom: 8 }}>
              <Text strong>{t('export.slip.payment')}</Text>{' '}
              {t(`common:status.payment.${lastOrder.paymentMethod}`, { defaultValue: lastOrder.paymentMethod })}
            </div>
            <div style={{ marginBottom: 8 }}>
              <Text strong>{t('export.slip.date')}</Text> {dateTime(lastOrder.createdAt)}
            </div>
            <Divider style={{ margin: '8px 0' }} />
            ...
            <div style={{ marginTop: 32, display: 'flex', justifyContent: 'space-between' }}>
              <div style={{ textAlign: 'center' }}>
                <div>{t('export.slip.staff')}</div>
                <div style={{ marginTop: 40 }}>________________</div>
              </div>
              <div style={{ textAlign: 'center' }}>
                <div>{t('export.slip.customerSign')}</div>
                <div style={{ marginTop: 40 }}>________________</div>
              </div>
            </div>
          </div>
