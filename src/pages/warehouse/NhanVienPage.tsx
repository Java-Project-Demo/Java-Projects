...
import {
  App, Avatar, Breadcrumb, Button, Card, Form, Input, Modal,
  Select, Space, Table, Tag, Tooltip, Typography,
} from 'antd'
import { HomeOutlined, PlusOutlined, EditOutlined, LockOutlined, UserOutlined } from '@ant-design/icons'
...
interface FormValues { fullName: string; roleName: string; status?: string }
...
const NhanVienPage = () => {
  ...
  const { data: page, isLoading } = useGetUsersQuery({ page: 0, size: 100 })
  ...
  const filtered = users.filter((u) => {
    const ms = !search || (u.fullName ?? '').toLowerCase().includes(search.toLowerCase()) ||
  ...
  const columns: ColumnsType<User> = [
    ...
    {
      title: 'Điện thoại', dataIndex: 'phoneNumber', key: 'phone', width: 130,
      render: (v: string | null) => v ?? '—',
    },
    {
      title: 'Vai trò', dataIndex: 'role', key: 'role', width: 140,
      render: (v: string) => {
        const cfg = ROLE_CONFIG[v] ?? { label: v, color: 'default' }
    ...
    },
    {
      ...
      render: (v: string | null, r: User) =>
        r.isDeleted ? <Tag color='red'>Đã xoá</Tag>
          : <Tag color={v === 'ACTIVE' || !v ? 'green' : 'orange'}>{v === 'ACTIVE' || !v ? 'Hoạt động' : 'Khoá'}</Tag>,
    },
    {
      title: 'Đặt lại MK', key: 'resetPwd', width: 90,
  ...
  return (
    <div>
      <Breadcrumb style={{ marginBottom: 16 }}
        items={[{ href: '/', title: <HomeOutlined /> }, { title: 'Nhân viên' }]} />

      <Card
        style={{ borderRadius: 12, boxShadow: '0 2px 10px rgba(0,0,0,0.07)' }}
        title={<Space><UserOutlined style={{ color: PRIMARY }} /><span>Quản lý nhân viên</span></Space>}
      ...
      >
        {tempPassword ? (
          <div style={{ textAlign: 'center', padding: '16px 0' }}>
            ...
            <div style={{ fontSize: 24, fontWeight: 700, color: PRIMARY, margin: '12px 0', letterSpacing: 2 }}>
              {tempPassword}
            </div>
            <Text type='secondary'>Nhân viên cần đổi mật khẩu khi đăng nhập lần tiếp theo.</Text>
          </div>
        ) : (
          <Text>Bạn có chắc muốn đặt lại mật khẩu cho <strong>{resetTarget?.username}</strong>?
            Mật khẩu tạm thời sẽ được tạo ngẫu nhiên.</Text>
        )}
      </Modal>
    </div>
  )
